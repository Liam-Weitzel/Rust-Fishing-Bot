...
Func ArrayDisplayEx_ColumnFormats( ByRef $hNotifyFunc, $iDimension, $i1dColumns, ByRef $aSort_Rows, ByRef $aSort_Cols )
	Switch True
		...
		Case IsDllStruct( $aSort_Rows )
			$hNotifyFunc = $iDimension = 1 ? $i1dColumns = 0 ? _ArrayDisplayEx_WM_NOTIFY14 : _ArrayDisplayEx_WM_NOTIFY14_Cols : _ArrayDisplayEx_WM_NOTIFY15 ; Sort rows through struct index, column text formatting
...
Func _ArrayDisplayEx_WM_NOTIFY10( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	...
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $aFormats, $iFmtPars, $aDisplay[100], $iFrom, $iTo
	...
	Switch DllStructGetData( DllStructCreate( "struct; hwnd hWndFrom;uint_ptr IDFrom;int Code; endstruct", $lParam ), "Code" )
		...
		Case  -12, -320, -321 ; $NM_CUSTOMDRAW, $HDN_ITEMCHANGINGW, $HDN_ITEMCHANGEDW
			Return
...
Func _ArrayDisplayEx_WM_NOTIFY10_Cols( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $aFormats, $iFmtPars, $aDisplay[100], $iFrom, $iTo, $i1dColumns, $iRowCount
...
Func _ArrayDisplayEx_WM_NOTIFY11( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $aFormats, $iFmtPars, $iColCount, $aDisplay[1][1], $iFrom, $iTo
...
Func _ArrayDisplayEx_WM_NOTIFY12( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $aSort_Rows, $bDef_Sort, $aFormats, $iFmtPars, $aDisplay[100], $iFrom, $iTo
...
Func _ArrayDisplayEx_WM_NOTIFY12_Cols( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $aSort_Rows, $bDef_Sort, $aFormats, $iFmtPars, $aDisplay[100], $iFrom, $iTo, $i1dColumns
...
Func _ArrayDisplayEx_WM_NOTIFY13( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $aSort_Rows, $bDef_Sort, $aFormats, $iFmtPars, $iColCount, $aDisplay[1][1], $iFrom, $iTo
...
Func _ArrayDisplayEx_WM_NOTIFY14( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $tSort_Rows, $bDef_Sort, $aFormats, $iFmtPars, $aDisplay[100], $iFrom, $iTo
	...
	Switch DllStructGetData( DllStructCreate( "struct; hwnd hWndFrom;uint_ptr IDFrom;int Code; endstruct", $lParam ), "Code" )
		...
		Case 0
			If $iIdx <> $iIndex Then ; Get data from $aDataDisplay_Info
				...
				$iRows = DllStructGetSize( $tSort_Rows ) / 4
				$bDef_Sort = 1 - $aDataDisplay_Info[$iIdx][9] ; 1 - $bSort_Reverse
				$aFormats = $aDataDisplay_Info[$iIdx][5]
				$iFmtPars = UBound( $aFormats, 2 ) - 1
			Else
...
; 1d array, sort rows through struct index, column text formatting, display array in multiple columns
Func _ArrayDisplayEx_WM_NOTIFY14_Cols( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $tSort_Rows, $bDef_Sort, $aFormats, $iFmtPars, $aDisplay[100], $iFrom, $iTo, $i1dColumns
	...
	Switch DllStructGetData( DllStructCreate( "struct; hwnd hWndFrom;uint_ptr IDFrom;int Code; endstruct", $lParam ), "Code" )
		...
		Case -113 ; $LVN_ODCACHEHINT
			...
			For $i = $iFrom To $iTo
				...
				Switch $aFormats[0][$iFmtPars]
					Case 2
						For $k = 0 To $i1dCols
							$aDisplay[$i-$iFrom][$k] = $aFormats[0][1]( $aArray[DllStructGetData($tSort_Rows,1,$bDef_Sort?$j+$k+1:$iRows-$j-$k+1)] )
						Next
					Case 3
						For $k = 0 To $i1dCols
							$aDisplay[$i-$iFrom][$k] = $aFormats[0][1]( $aArray[DllStructGetData($tSort_Rows,1,$bDef_Sort?$j+$k+1:$iRows-$j-$k+1)], $aFormats[0][2] )
						Next
					Case 4
						For $k = 0 To $i1dCols
							$aDisplay[$i-$iFrom][$k] = $aFormats[0][1]( $aArray[DllStructGetData($tSort_Rows,1,$bDef_Sort?$j+$k+1:$iRows-$j-$k+1)], $aFormats[0][2], $aFormats[0][3] )
						Next
					Case 5
						For $k = 0 To $i1dCols
							$aDisplay[$i-$iFrom][$k] = $aFormats[0][1]( $aArray[DllStructGetData($tSort_Rows,1,$bDef_Sort?$j+$k+1:$iRows-$j-$k+1)], $aFormats[0][2], $aFormats[0][3], $aFormats[0][4] )
						Next
					Case 6
						For $k = 0 To $i1dCols
							$aDisplay[$i-$iFrom][$k] = $aFormats[0][1]( $aArray[DllStructGetData($tSort_Rows,1,$bDef_Sort?$j+$k+1:$iRows-$j-$k+1)], $aFormats[0][2], $aFormats[0][3], $aFormats[0][4], $aFormats[0][5] )
						Next
					Case 7
						For $k = 0 To $i1dCols
							$aDisplay[$i-$iFrom][$k] = $aFormats[0][1]( $aArray[DllStructGetData($tSort_Rows,1,$bDef_Sort?$j+$k+1:$iRows-$j-$k+1)], $aFormats[0][2], $aFormats[0][3], $aFormats[0][4], $aFormats[0][5], $aFormats[0][6] )
						Next
				EndSwitch
			Next
			Return
		...
		Case 0
			If $iIdx <> $iIndex Then ; Get data from $aDataDisplay_Info
				...
				$aArray = $aDataDisplay_Info[$iIdx][0]
				$tSort_Rows = $aDataDisplay_Info[$iIdx][3]
				$iRows = DllStructGetSize( $tSort_Rows ) / 4 - 1
				$bDef_Sort = 1 - $aDataDisplay_Info[$iIdx][9] ; 1 - $bSort_Reverse
				$i1dColumns = $aDataDisplay_Info[$iIdx][12]
				$aFormats = $aDataDisplay_Info[$iIdx][5]
				$iFmtPars = UBound( $aFormats, 2 ) - 1
...
Func _ArrayDisplayEx_WM_NOTIFY15( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $tSort_Rows, $bDef_Sort, $aFormats, $iFmtPars, $iColCount, $aDisplay[1][1], $iFrom, $iTo
...
Func _ArrayDisplayEx_WM_NOTIFY16( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $aSort_Cols, $aFormats, $iFmtPars, $iColCount, $aDisplay[1][1], $iFrom, $iTo
...
Func _ArrayDisplayEx_WM_NOTIFY17( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $aSort_Rows, $bDef_Sort, $aSort_Cols, $aFormats, $iFmtPars, $iColCount, $aDisplay[1][1], $iFrom, $iTo
...
Func _ArrayDisplayEx_WM_NOTIFY18( $hWnd, $iMsg, $wParam, $lParam, $iIndex, $pData )
	If $iMsg <> 0x004E Then Return DllCall( "comctl32.dll", "lresult", "DefSubclassProc", "hwnd", $hWnd, "uint", $iMsg, "wparam", $wParam, "lparam", $lParam )[0] ; 0x004E = $WM_NOTIFY
	Local Static $tText = DllStructCreate( "wchar[260]" ), $pText = DllStructGetPtr( $tText ), $iIdx = 0, $aArray, $iRows, $tSort_Rows, $bDef_Sort, $aSort_Cols, $aFormats, $iFmtPars, $iColCount, $aDisplay[1][1], $iFrom, $iTo
